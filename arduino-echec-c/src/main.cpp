...
// //allocations des differentes ressources qui vont etre ajouté par la suite a la pool et accessible de maniere unique n'importe où dans le code
...
void loop()
{
    ...
    {
        Serial.print(myTransfer.packet.rxBuff[0]);
        Serial.print(myTransfer.packet.rxBuff[1]);
        Serial.print(myTransfer.packet.rxBuff[2]);
    ...
    if (must)
    {
        myTransfer.txObj(test, sizeof(test), sendSize);
        sendSize += sizeof(test);
